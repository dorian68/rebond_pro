...
runner("roadmap_2_drive_smoke", async () => {
  ...
  const actionNames = ["getRoadmap2DriveStatus", "connectRoadmap2Drive", "provisionRoadmap2Drive", "listRoadmap2DriveFiles", "createRoadmap2NodeDriveResources", "syncRoadmap2DrivePermissions"];
  ...
  assert(client.includes("Connecter Google Drive") && client.includes("Changer / reconnecter") && client.includes("Afficher le contenu") && detail.includes("Créer automatiquement"), "Le parcours connexion, récupération de compte, explorateur et création par nœud doit être visible dans Roadmap 2.");
