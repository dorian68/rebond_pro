...
- Connecteurs Socrate via Composio : Google Calendar et Microsoft Calendar en lecture seule ; Google Drive, OneDrive et SharePoint en recherche/import de fichiers ; Outlook en création de brouillons uniquement. Pour le super-admin sur Roadmap 2, Gmail permet aussi la lecture et l'envoi contrôlé : aperçu obligatoire, validation humaine signée et à usage unique, compte connecté épinglé et registre d'idempotence durable. Deux périmètres UX : **Mes connexions** (utilisateur) et **Connexions du centre** (organisation). Configuration dans Paramètres → Connecteurs. Testé par `smoke:connectors` et `smoke:roadmap-2:agentic-gmail`.
...
- Les connecteurs externes n'exposent aucune écriture calendrier. Outlook reste limité au brouillon. L'envoi Gmail est réservé au super-admin dans le contexte Roadmap 2 et exige une validation signée, à usage unique, après affichage du destinataire, de l'objet et du corps. L'import de fichier externe, la création de brouillon et toute mutation Roadmap 2 exigent aussi une validation humaine.
