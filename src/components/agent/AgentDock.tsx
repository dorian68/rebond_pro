...
export function AgentDock({
  ...
  const onFilePick = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const ALLOWED = ["image/jpeg", "image/png", "image/webp", "image/gif", "application/pdf", "text/csv", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"];
    const valid = files.filter((f) => (ALLOWED.includes(f.type) || /\.(docx|xlsx|csv)$/i.test(f.name)) && f.size <= MAX);
          ...
          >
            ...
            <div style={{ borderTop: "1px solid var(--border-2)", padding: 12, flexShrink: 0 }}>
              ...
              <div style={{ display: "flex", gap: 6, alignItems: "flex-end", background: "var(--surface-3)", borderRadius: 14, padding: "6px 8px", border: "1px solid var(--border)" }}>
                ...
                <input ref={fileInputRef} type="file" multiple accept="image/jpeg,image/png,image/webp,image/gif,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,text/csv,.docx,.xlsx,.csv" onChange={onFilePick} style={{ display: "none" }} />
