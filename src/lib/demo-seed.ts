...
export async function loadDemoData(organizationId: string): Promise<void> {
  ...
  const [claire, julien, sarah, thomas] = await Promise.all([
    prisma.trainer.create({ data: { organizationId, firstName: "Claire", lastName: "Martin", initials: "CM", specialities: ["Excel", "Power BI", "Tableaux de bord"], color: "#5850ec", email: "claire.martin@demo.local", yearsExperience: 12, bio: "Experte bureautique et data-visualisation, Claire accompagne depuis 12 ans les équipes administratives et financières des PME pour automatiser leurs reportings et gagner un temps précieux au quotidien." } }),
  ...
  const ia = await prisma.formation.create({
    data: { organizationId, title: "IA pour fonctions administratives", slug: "ia-admin", category: "Intelligence artificielle", shortDescription: "Utilisez l'IA pour automatiser vos tâches admin.", durationDays: 1, durationHours: 7, price: EUR(450), modality: "DISTANCIEL", level: "DEBUTANT", color: "#6a5cf0", status: "PUBLIE", isPublic: true, publicSlug: `demo-ia-${organizationId.slice(0, 8)}`, eligibleTrainers: { create: [{ trainerId: julien.id }] } },
  });
