...
export async function resolveDocumentTemplate(input: {
  ...
  const automatic = explicit ?? await prisma.documentTemplate.findFirst({
    where: {
      type: type as never,
      status: "ACTIVE",
      engine: "DOCX",
      sourceFileUrl: { not: null },
      OR: [{ organizationId: ctx.organizationId, isDefault: true }, { organizationId: ctx.organizationId }, { organizationId: null, isDefault: true }, { organizationId: null }],
    },
    orderBy: [
      { isDefault: "desc" },
      { organizationId: "desc" },
