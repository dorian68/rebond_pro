...
export function ManualGenerateForm({ sessions, templates }: { sessions: SessionOption[]; templates: TemplateOption[] }) {
  ...
  function runPreflight() {
    ...
    fd.set("manualOverrides", manualOverrides);
  ...
  return (
    <form action={action} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div>
        <label className="field-label">Compléments rapides optionnels</label>
        <textarea
          className="input"
          name="manualOverrides"
          rows={3}
          value={manualOverrides}
          onChange={(e) => { setManualOverrides(e.target.value); resetPreflight(); }}
          placeholder='{"funding_organization":"OPCO", "quote_number":"DEV-2026-001"}'
...
function PreflightCard({ preflight }: { preflight: DocumentPreflight }) {
    ...
    <div style={{ padding: "12px 14px", borderRadius: 10, background: "var(--surface-3)", border: "1px solid var(--border)", display: "grid", gap: 8 }}>
      ...
      {preflight.missingVariables.length > 0 ? (
        <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
          {preflight.missingVariables.slice(0, 8).map((m) => <span key={m.key} className="badge badge-warn">{m.label}</span>)}
        </div>
      ) : null}
