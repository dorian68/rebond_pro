...
export default async function TrainerLayout({ children }: { children: React.ReactNode }) {
  ...
  if (ctx.role !== "TRAINER" && ctx.role !== "OWNER" && ctx.role !== "ADMIN") redirect("/dashboard");
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", display: "flex", flexDirection: "column" }}>
      <header style={{ background: "#fff", borderBottom: "1px solid var(--border)", position: "sticky", top: 0, zIndex: 20 }}>
        <div style={{ maxWidth: 860, margin: "0 auto", padding: "0 20px", height: 60, display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 32, height: 32, borderRadius: 8, background: "linear-gradient(140deg,#2f9488,#2469a6)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700 }}>
              {initial}
            </div>
            <span style={{ fontSize: 13, fontWeight: 600, color: "var(--ink-2)" }}>{ctx.name ?? ctx.email}</span>
            {(ctx.role === "OWNER" || ctx.role === "ADMIN") && (
              <Link href="/dashboard" className="btn btn-secondary btn-sm">
                <Icon name="layout-dashboard" size={14} /> Back-office
              </Link>
            )}
            <form action={logoutAction}>
              <button type="submit" className="btn btn-ghost btn-icon" title="Déconnexion">
      ...
      <main style={{ flex: 1, maxWidth: 860, margin: "0 auto", width: "100%", padding: "28px 20px" }}>
        {children}
      </main>
