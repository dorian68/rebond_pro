            ...
            {sorted.map((node) => {
              ...
              return (
                <tr key={node.id} className={`${overdue ? styles.listOverdue : ""} ${node.status === "blocked" ? styles.listBlocked : ""}`}>
                  ...
                  <td data-label="Statut"><select disabled={node.status === "archived"} className={styles[`selectStatus_${node.status}`]} aria-label={`Statut de ${node.title}`} value={node.status} onChange={(event) => void actions.quickUpdate(node, { status: event.target.value as Roadmap2NodeDto["status"] })}>{ROADMAP2_STATUSES.filter((status) => status !== "archived" || node.status === "archived").map((status) => <option key={status} value={status}>{ROADMAP2_STATUS_LABELS[status]}</option>)}</select></td>
